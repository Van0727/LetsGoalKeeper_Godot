...
func _run() -> void:
	...
	_assert(lane.judgement_label.text.begins_with("MISS · "), "Miss判定使用全大写文案")
	...
	_assert(lane.judgement_label.text.begins_with("GOOD · "), "Good判定使用全大写文案")
	var good_color := lane.judgement_label.get_theme_color("font_color")
	lane.show_judgement({"grade_name": "Perfect", "error_ms": 0.0})
	_assert(lane.judgement_label.text.begins_with("GREAT · "), "内部Perfect判定对玩家显示全大写GREAT")
	...
	_assert(shared_outline_size >= 3 and shared_outline_size == lane.judgement_label.get_theme_constant("outline_size"), "三种判定共用清晰粗描边")
	...
	_assert(not battle.get_node("RhythmLaneArt").visible, "圆圈隐藏旧节奏底图")
	...
	_assert(battle.get_node("RhythmLaneArt").visible, "波形保留原底图")
