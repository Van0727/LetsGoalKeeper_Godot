...
func _run() -> void:
	...
	# 底部角色 HUD 与核心角色状态同源：数值、进度条和临时状态不能出现不同步。
	_assert_equal(battle_screen.player_health_label.text, "100/100", "角色 HUD 显示当前生命与上限")
	_assert_equal(battle_screen.player_health_bar.value, 100.0, "生命条使用当前生命值")
	_assert_equal(battle_screen.player_shield_label.text, "0/20", "零护盾仍保留固定容量底板")
	...
	battle_screen.controller.player.gain_shield(8)
	battle_screen._refresh_all()
	_assert_equal(battle_screen.player_shield_label.text, "8/20", "护盾变化同步到角色 HUD")
	_assert_true(battle_screen.strength_status_label.visible, "力量或虚弱出现时显示状态槽")
	_assert_equal(battle_screen.strength_status_label.text, "↑2", "力量状态显示剩余回合")
	_assert_true(battle_screen.bleed_status_label.visible, "流血出现时显示减益槽")
